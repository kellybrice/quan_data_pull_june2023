...
WHERE ((( convert_timezone(stores.store_region_timezone, f_order_items.order_completed_at)  ) >= (TIMESTAMP '2022-05-01') AND ( convert_timezone(stores.store_region_timezone, f_order_items.order_completed_at)  ) < (TIMESTAMP '2023-05-01'))) AND (NOT (f_order_items.is_free_order ) OR (f_order_items.is_free_order ) IS NULL) AND (f_order_items.is_order_complete ) AND (stores.store_category ) = 'US-Ecom'
GROUP BY
    1,
    2
union all
SELECT
    "f_sessions"."dma_name" AS "market",
    (DATE(convert_timezone(stores.store_region_timezone, f_sessions.utc_session_start_at) )) AS "date",
    COUNT(DISTINCT f_sessions.session_user_id) AS "count",
    'site traffic' as "event"
FROM
    "elsa"."f_sessions" AS "f_sessions"
    LEFT JOIN "elsa"."dim_store_union" AS "stores" ON "f_sessions"."store_code" = "stores"."store_code"
WHERE ((( convert_timezone(stores.store_region_timezone, f_sessions.utc_session_start_at)  ) >= (TIMESTAMP '2022-05-01') AND ( convert_timezone(stores.store_region_timezone, f_sessions.utc_session_start_at)  ) < (TIMESTAMP '2023-05-01'))) AND "stores"."store_category" = 'US-Ecom'
GROUP BY
    1,
    2
ORDER BY
